...
sub reportLastOutputs {
    ...
    for my $runOutputType (qw(unisonCaptureLog unisonStdErrFile)) {
        ...
        open( FH, $file )
          or next;    # Error appending contents of $account->{$runOutputFile}
        my $text = do { local ($/); <FH> };
        $account->{report} .= "====$runOutputType===\n$text\n===end===\n";
        close FH;
        report( $account,
            "Sweeping $runOutputType ($file) into report and deleting $file\n" )
          if ( $account->{debug} > 2 );
        if ( $account->{debug} > 2 ) {
...
sub syncAccount {
    ...
    if ( !defined $account ) {
        ...
        report( $accounts->{$accountName},
            "No such account '$accountName' - skipping\n" );
        next;
    }
    ...
    foreach my $key (%$options) {
        ...
        if ( !defined $account->{$key} ) {
            ...
            report( $account,
"Warning: global key $value overridden by local account $accountName value "
                  . $account->{$key}
                  . "\n" );
        }
    }
    ...
    expandAccountVariables( $account, 'unisonStdErrFile', 'unisonCaptureLog',
        'unisonLogfile' );
...
sub syncDir {
  ...
  # TODO: test you can set this to '' and have that override separation of webs.
    ...
    my $optionalClientParentSlash =
      optionalParentSlash( $account->{clientParentWeb} );
    my $optionalServerParentSlash =
      optionalParentSlash( $account->{serverParentWeb} );
...
sub syncFileSet {
    ...
    unless ( $account->{dryrun} ) {
        ...
        report( $account,
            "(turn debug >= 2 to see cmd that would be executed)\n" )
...
sub getSyncFileSetCommand {
    ...
    if ( $account->{serverSite} ) {
        $optionalServerSpec =
          $account->{protocol} . '://' . $account->{serverSite} . '/';
        @optionalSshCmd =
          ( "-sshcmd", $account->{plinkTempLauncherScriptFile} );
    }

    my $clientFileSet = $account->{clientRoot} . '/' . $clientDir;
    my $serverFileSet =
      $optionalServerSpec . $account->{serverRoot} . '/' . $serverDir;

    my @unisonArgs = ( @optionalSshCmd, $account->{unisonOptions} );

    my $unisonCaptureErrors = $account->{unisonCaptureErrors};
    my @cmd                 = (
        $account->{clientUnisonExecutable},
        $clientFileSet, $serverFileSet, @unisonArgs, $unisonCaptureErrors
    );
...
sub plinkLauncherScriptContents {
    my ($account) = @_;
    return
"\@\"$account->{plinkExecutable}\" $account->{serverSite} -i \"$account->{clientServerPrivateKey}\" -l $account->{serverAccount} -ssh $account->{serverUnisonExecutable} -server -contactquietly";
}
...
sub deletePlinkLauncherScript {
    my ($account) = @_;
    if ( $account->{serverSite} ) {
        unlink $account->{plinkTempLauncherScriptFile}
          unless ( $account->{debug} > 2 );
    }
}
...
sub doSubstitutions {
    ...
    foreach my $key ( keys %$account ) {
        ...
        $text =~ s/%$key%/$value/g
          if ($value)
          ;    #SMELL - this needs to be a literal key, e.g. pass key = '{$}'
    }
    return $text;
}
